<?php
gatekeeper();
//echo "UID".elgg_get_logged_in_user_guid();
	...
	elseif ($step=='send_invites')
		...
		if (count($ers)==0)
			...
			foreach ($selected_contacts as $email=>$name)
					{
			 	 /* Added By Rijo Joy  for adding to contact list*/
			...
			if ($sendMessage===-1)
				...
				$oks['mails']="Contact Added Successfully";
				}
			...
			else $oks['internal']="Contacts Added successfully!";
			$done=true;
			}
		}
	}
	...
	if ($step=='send_invites')
		...
		if ($inviter->showContacts())
			...
			if (count($contacts)==0)
				...
				<input type='submit' name='send' value='Add Contacts' class='thButton'></td></tr>";
				}
			$contents.="</table>";
			}
