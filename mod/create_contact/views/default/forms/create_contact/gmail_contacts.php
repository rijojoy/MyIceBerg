...
"enabled" => true,
"keys" => array ( "id" => "923402212598.apps.googleusercontent.com", "secret" => "hbVtCfaTz2xidihJ_j15l0Tn" )
), 
"Facebook" => array (
...
echo elgg_echo('Import Contacts');
$hybridauth = new Hybrid_Auth( $config_array );
$google_adapter = $hybridauth->authenticate( "Google" );
$google_contacts = $google_adapter->getUserContacts();
$connected = $google_adapter->isUserConnected();
if($connected == false)
{
$google_adapter->logout();
}
echo '<table>';
foreach ($google_contacts as $google_contact)
{
	
	echo '<tr>';
	echo '<td width="55"><img src="'.$google_contact->photoURL.'" style="border: 1px solid #888888; padding: 5px;" />';
	echo '<td>';
	echo '<a href="'.$google_contact->profileURL.'" target="_blank">'.$google_contact->displayName.' &nbsp;<span class="label success">User ID:'. $google_contact->identifier.'</span><br><br>';
	echo '<em>'.$google_contact->profileURL.'</em>';
	echo '</td>';
