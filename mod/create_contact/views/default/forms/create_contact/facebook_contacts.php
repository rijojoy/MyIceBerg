...
if(isset($_POST['submit']))
{
	...
	$fbdataChecked = get_input('id');
	foreach( $fbdataChecked as $fbdata)
	 {  
		...
		// Save contact type facebook
		$contact->contact_type = $fbd[3];
		
		// Save facebook unique id
		$contact->import_id = $fbd[0];
...
"providers" => array (
"Twitter" => array (
"enabled" => false, "keys" => array ( "key" => "", "secret" => "" )
),
"Facebook" => array (
...
//echo '<pre>';
//print_r(get_input('id'));
echo '<form name="frm" action="" method="post" >';
echo '<table>';
echo '<tr><td> <input type="checkbox" name="checkbox" value="checkbox" onClick="checkUncheckAll(this);"</td><td><input  name="submit" type="submit" value="Add to Contacts" /></td></tr>';
foreach ($facebook_contacts as $k => $facebook_contact)
{
		
	$name = explode(" ", $facebook_contact->displayName);
	$fname = $name[0];
	$lname = $name[1];
	$fbdata = array(
	                "id" => $facebook_contact->identifier,
					"fname" => $name[0], 
					"lname" => $name[1], 
					"contact_type" => "facebook",
					"photo_url" => $facebook_contact->photoURL,
					"profile_url" => $facebook_contact->profileURL
					);
	$fb_implode = implode(",",$fbdata); 
	
	echo '<tr>';
	echo '<td width="55">';
	echo '<input type="checkbox" name="id[]" value="'.$fb_implode.'" />';
	//echo '<input type="hidden" name="id[]" value="'.$name[0].'" />';
